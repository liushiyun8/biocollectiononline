...
import com.emptech.biocollectiononline.camera.CameraGLSurfaceView.CameraTextureListener;
...
public abstract class CameraGLRendererBase implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener {
    ...
    protected int mCameraIndex = CameraRenderer.CAMERA_ID_ANY;
    ...
    protected PreviewAndPictureCallback previewAndPictureCallback;
...
//        float[] vertex = TextureRotationUtil.scaleVertex(vertices, 1.5f);
        vert.put(vertices).position(0);
        float[] rotation = TextureRotationUtil.getRotation(Rotation.NORMAL, true, true);
//        texOES.put(texCoordOES).position(0);
        texOES.put(rotation).position(0);
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        synchronized(this) {
            ...
            if(texListener != null) {
                ...
                if(modified) {
                    texDraw[0]=drawImage();
                    Log.d(LOGTAG, "draw texDraw YUV -> screen"+",texDraw[0]"+texDraw[0]);
                    // texDraw -> screen
...
//                int tex=drawImage();
    ...
    public void setPreviewAndPictureCallback(PreviewAndPictureCallback callback) {
        this.previewAndPictureCallback = callback;
    }
