...
public class UsbDeviceDataExchangeImpl {
    ...
    public interface onUSBListener {
        void usbState(int MessageDevice);
    }

    private onUSBListener usbListener;

    public void setonUSBListener(onUSBListener usbListener) {
    ...
    public static final int ACTION_USB_DEVICE_ATTACHED = 257;
    public static final int ACTION_USB_DEVICE_DETACHED = 258;
    ...
    private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Log.d(log_tag, "收到广播onReceive: action: " + action);
            if (ACTION_USB_PERMISSION.equals(action)) {
                if (mUsbDevice != null) {
                    synchronized (mPermissionIntent) {
                        UsbDevice device = (UsbDevice) intent.getParcelableExtra(UsbManager.EXTRA_DEVICE);
                        Log.d(log_tag, "onReceive: device: " + device.toString());
                        if (mUsbDevice.equals(device)) {
                            if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
                                if (usbListener != null) {
                                    usbListener.usbState(MESSAGE_ALLOW_DEVICE);
                                }
                            } else {
                                if (usbListener != null) {
                                    usbListener.usbState(MESSAGE_DENY_DEVICE);
                                }
                            }
                        }
                    }
    ...
    private final BroadcastReceiver mUsbAttachedReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (UsbManager.ACTION_USB_DEVICE_ATTACHED.equals(action)) {
                Log.e(log_tag, "USB device is Attached: " + device.toString());
                if (usbListener != null) {
                    usbListener.usbState(ACTION_USB_DEVICE_ATTACHED);
                }
            } else if (UsbManager.ACTION_USB_DEVICE_DETACHED.equals(action)) {
                Log.e(log_tag, "USB device is Detached: " + device.toString());
                if (usbListener != null) {
                    usbListener.usbState(ACTION_USB_DEVICE_DETACHED);
                }
            }
        }
    };
    ...
    public UsbDeviceDataExchangeImpl(Context ctx) {
        ...
        isregisterPermissionReceiver=true;
        LogUtils.v(AppConfig.MODULE_APP, "注册USB权限监听;");
    }
    ...
    public void registerUSBReceiver() {
        ...
        context.registerReceiver(mUsbAttachedReceiver, filter);
    ...
    private boolean isregisterPermissionReceiver = false;

    public void unRegisterUSBReceiver() {
        if (isregisterUSBReceiver && mUsbAttachedReceiver != null) {
    ...
    public void Destroy() {
        ...
        if(isregisterPermissionReceiver){
            context.unregisterReceiver(mUsbReceiver);
            isregisterPermissionReceiver=false;
        }
    ...
    public boolean OpenDevice(int instance, boolean is_activity_thread) {
        synchronized (this) {
            Log.d(log_tag, "OpenDevice: " + instance + ", usb_ctx == null(" + Boolean.toString(usb_ctx == null) + ")");
            mInstance = instance;
            ...
            if (usb_ctx == null) {
                ...
                List<UsbDevice> usbDeviceList = new LinkedList<>();

                int index = 0;

                while (deviceIterator.hasNext()) {
                    UsbDevice device = deviceIterator.next();
                    Log.d(log_tag, "UsbDevice: " + device.toString());
                ...
                ListIterator<UsbDevice> usbDeviceListIterator = usbDeviceList.listIterator();
                while (usbDeviceListIterator.hasNext()) {
                    UsbDevice device = usbDeviceListIterator.next();
//                    Log.d(log_tag, "usbDeviceListIterator.next: " + device.toString());
                }

                while (usbDeviceListIterator.hasPrevious()) {
                    UsbDevice device = usbDeviceListIterator.previous();
                    Log.d(log_tag, "usbDeviceListIterator.previous: " + device.getDeviceName());

                    if (IsFutronicDevice(device.getVendorId(), device.getProductId())) {
                        Log.d(log_tag, "index: " + index + ", instance: " + instance);
                        if (index < instance) {
                            index++;
                            continue;
                        }
                        ...
                        /*yzq: add for fix bug 20180103_104404 */
                        break;
                    }
                }
    ...
    public boolean OpenDevice(UsbDevice device, boolean is_activity_thread) {
        synchronized (this) {
            Log.d(log_tag, "OpenDevice: " + device.toString() + ", usb_ctx == null(" + Boolean.toString(usb_ctx == null) + ")");
            if (usb_ctx == null) {
                pending_open = false;
                if (IsFutronicDevice(device.getVendorId(), device.getProductId())) {
                    mUsbDevice = device;
